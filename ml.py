...
import torch
def decompose_defender(dense_index,model):
    W_List = []
    b_List = []
    for i in dense_index:
        Wi = model.classifier[i].weight.tolist()
        Wi = sfix.input_tensor_via(1, Wi)
        W_List.append(Wi)

        bi = model.classifier[i].bias.tolist()
        bi = sfix.input_tensor_via(1, bi)
        b_List.append(bi)
    return W_List,b_List

def one_hot_generate(vector_length,index):
    e = sfix.Array(vector_length)
    e[index]=sfix(1)
    return e

def defender_forward(z,W_List,b_List):
    drelu_List = []
    for i in range(len(W_List)-1):
        z = W_List[i].dot(z)
        z[:] += b_List[i][:]
        drelu_List.append(sfix.Array(z.shape[0]))
...
def pi_softmax(x):
    ...
    for i in range(n):
        ...
        s += ex
        exp_list.append(ex)
    new_array = sfix.Array(n)
    s = 1/s
    for i in range(n):
        new_array[i] = exp_list[i] * s
    return new_array
...
def l2_norm(a):
    ...
    def _(i):
        norm[0] += (a[i][0]) * (a[i][0])
    b = mpc_math.sqrt(norm[0])  # I want to return this
    # For debugging
    #print_ln("Norm: %s %s", norm[0].reveal(), b.reveal())
    return b